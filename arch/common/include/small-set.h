...
public:
	...
	class iterator {
		...
		public:
			iterator(smallset_t &target) : _target(target), _ptr(0) { advance(); }
			
			bool operator==(const iterator &other) { return _ptr == other._ptr; }
			bool operator!=(const iterator &other) { return _ptr != other._ptr; }
			innertype &operator*() { return _target._data[_ptr]; }
